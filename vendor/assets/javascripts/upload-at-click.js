...
function upclick(params)
{
    ...
    frame.onload_callback =
        function()
        {
            ...
            var authenticity_token  = window.parent.jQuery('meta[name="csrf-token"]').attr('content');

        		var remember_token      = $('<input type="hidden" value="'+$.cookie('remember_token')+'" name="remember_token"/>');

            $(form).append('<input type="hidden" value="'+authenticity_token+'" name="authenticity_token"/>');
        		$(form).append(remember_token);
            ...
            var onchange_callback =
                function(e)
                {
                    ...
                    var stop_send = null;
                    if (onstart){
                      send_file = onstart(input.value, element);
                    }
                    if (send_file != false)
                      form.submit();
                };
